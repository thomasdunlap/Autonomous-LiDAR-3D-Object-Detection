...
def close_current_window(vis):
    vis.close()

def destroy_vis(vis):
    vis.destroy_window()
    raise StopIteration # loop_over_dataset hangs if StopIteration not raised
    
# visualize lidar point-cloud
def show_pcl(pcl):

    print("task ID_S1_EX2")

    # initialize open3d with key callback and create window
    vis = o3d.visualization.VisualizerWithKeyCallback()
    vis.create_window(window_name="LiDAR Visualization")
    vis.register_key_callback(68, destroy_vis) # key-code 68 is "d" for "destroy"
    ...
    # for the first frame, add the pcd instance to visualization using add_geometry; for all other frames, use update_geometry instead
    vis.add_geometry(pcd) 

    # visualize point cloud and keep window open until right-arrow is pressed (key-code 262)
    vis.register_key_callback(262, close_current_window)
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    print("task ID_S2_EX1")

    # compute bev-map discretization by dividing x-range by the bev-image height (see configs)
    bev_discrete_x = (configs.lim_x[1] - configs.lim_x[0]) / configs.bev_height
    
    # create a copy of the lidar pcl and transform all metrics x-coordinates into bev-image coordinates    
    lidar_pcl_cpy = np.copy(lidar_pcl)
    lidar_pcl_cpy[:, 0] = np.int_(np.floor(lidar_pcl_cpy[:, 0] / bev_discrete_x))

    # perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur
    bev_discrete_y =(configs.lim_y[1] - configs.lim_y[0]) / configs.bev_width

    lidar_pcl_cpy[:, 1] = np.int_(np.floor(lidar_pcl_cpy[:, 1] / bev_discrete_y) + (configs.bev_width + 1) / 2)
    ...
    deg1 = np.percentile(intensities, 1)
    deg99 = np.percentile(intensities, 99)
    intensities[intensities < deg1] = deg1
    intensities[intensities > deg99] = deg99
    intensities = (intensities - deg1) / (deg99 - deg1)
    
    intensity_map[np.int_(lidar_pcl_top[:, 0]), np.int_(lidar_pcl_top[:, 1])] = intensities
    ...
    min_h, max_h = configs.lim_z
    normalized_heights = (lidar_pcl_top[:, 2] - min_h) / (max_h - min_h)
